...
final class SvgCanvas
{
    ...
    public function shape(Shape $shape): void
    {
        ...
        //if ($this->isDep === false) {
        //    ($this->root)['width'] = ($shape->width / 20) . 'px';
        //    ($this->root)['height'] = ($shape->height / 20) . 'px';
        //}
    ...
    public function include(SpriteDefinition|ShapeDefinition $object, Matrix $matrix): void
    {
        $g = $this->g();

        // @todo éviter effets de bords
        $lastIsDep = $this->isDep;
        $lastId = $this->id;
        $lastG = $this->currentG;
        $this->currentG = null;
        $this->isDep = true;
    ...
    private function g(): SimpleXMLElement
    {
        ...
        $this->currentG = $g = $this->defs->addChild('g');
        $this->id = 'object-'.$this->lastId++;
        $g['id'] = $this->id;
